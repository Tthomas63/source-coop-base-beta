...
public:
	...
	{
		Precache();
		SetModel("models/weapons/w_gaussammo_mp.mdl");
		BaseClass::Spawn();
	}
	void Precache(void)
	{
		PrecacheModel("models/weapons/w_gaussammo_mp.mdl");
	}
	bool MyTouch(CBasePlayer *pPlayer)
	{
		if (ITEM_GiveAmmo(pPlayer, 25, "Uranium"))
		{
			if (g_pGameRules->ItemShouldRespawn(this) == GR_ITEM_RESPAWN_NO)
			{
				UTIL_Remove(this);
...
LINK_ENTITY_TO_CLASS(item_box_uranium, CItem_BoxUranium);
