...
// Both constants are set to only contain police model. Only one with almost complete working anims for now.
		...
		if ( HL2MPRules()->IsTeamplay() == false )
		{
			if ( GetModelPtr() == NULL )
			{
				...
				if ( ValidatePlayerModel( szModelName ) == false )
				{
					...
					Q_snprintf(szReturnString, sizeof(szReturnString), "cl_playermodel models/player/phoenix.mdl\n");
...
void CHL2MP_Player::SetPlayerTeamModel( void )
{
	...
	if ( modelIndex == -1 || ValidatePlayerModel( szModelName ) == false )
	{
		...
		szModelName = "models/player/phoenix.mdl";
...
void CHL2MP_Player::SetPlayerModel( void )
{
	...
	if ( ValidatePlayerModel( szModelName ) == false )
	{
		...
		if ( ValidatePlayerModel( pszCurrentModelName ) == false )
		{
			...
			pszCurrentModelName = "models/player/phoenix.mdl";
		}
...
void CHL2MP_Player::SetupPlayerSoundsByModel( const char *pModelName )
{
	...
	else if ( Q_stristr(pModelName, "player/police" ) )
	{
		m_iPlayerSoundType = (int)PLAYER_SOUNDS_METROPOLICE;
	}
	else if ( Q_stristr(pModelName, "player/combine" ) )
	{
...
void CHL2MP_Player::CreateGibbedRagdollEntity(void)
{
	if (m_hRagdoll)
	{
		UTIL_RemoveImmediate(m_hRagdoll);
	...
	if (!pRagdoll)
	{
		// create a new one
		pRagdoll = dynamic_cast< CHL2MPRagdoll* >(CreateEntityByName("hl2mp_ragdoll"));
		pRagdoll->SetModel("models/gibs/gibhead.mdl");
	}
...
void CHL2MP_Player::Event_Killed( const CTakeDamageInfo &info )
{
	...
	{
		CreateGibbedRagdollEntity(); // Create a ragdoll with a specific model
		//Blood
		for (int i = 0; i < random->RandomInt(1, 9); i++) // Random meatiness? ( Call these effects 1-9 times )
		{
			this->EmitSound("Player.Gib");
			Vector vecPos = this->GetAbsOrigin();
			vecPos.z = vecPos.z + 56; // Player should be 64 units high? 32 ish should be half height? // After testing set higher than `half`

			Vector randomVec = Vector(random->RandomInt(-20, 20), random->RandomInt(-20, 20), random->RandomInt(1, 13));
			Vector randomVecTwo = Vector(random->RandomInt(-20, 20), random->RandomInt(-20, 20), random->RandomInt(1, 13));
			Vector randomVecUp = Vector(random->RandomInt(-20, 20), random->RandomInt(-20, 20), random->RandomInt(0, 1));
			Vector randomVecDown = Vector(random->RandomInt(-20, 20), random->RandomInt(-20, 20), random->RandomInt(0, -1));

			UTIL_BloodSpray(vecPos, randomVec, BloodColor(), 10, FX_BLOODSPRAY_ALL); // EXPLOSION of blood drops
			UTIL_BloodDrips(vecPos, randomVec, BloodColor(), 10); // Couldn't really see?
			UTIL_BloodImpact(vecPos, randomVec, BloodColor(), 10); // Standard getting shot effect
			SpawnBlood(vecPos, randomVec, BloodColor(), info.GetDamage() * 5); //Idk what this does lets see

			Vector	vecSpot;
			Vector vecDir;
			trace_t	tr;

			vecSpot = this->GetAbsOrigin() + Vector(0, 0, 8);//move up a bit, and trace down.
			vecDir = vecSpot + Vector(0, 0, -24);
			UTIL_TraceLine(vecSpot, vecDir, MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);

			// Just try sponging some down? THIS WORKS
	...
	{
		//Blood
		for (int i = 0; i < random->RandomInt(1, 5); i++) // Random meatiness? ( Half as good as gib )
		{
			this->EmitSound("Player.Gib");
			Vector vecPos = this->GetAbsOrigin();
			vecPos.z = vecPos.z + 56; // Player should be 64 units high? 32 ish should be half height? // After testing set higher than `half`
			...
			UTIL_BloodSpray(vecPos, randomVecTwo, BloodColor(), 5, FX_BLOODSPRAY_ALL); //Second set of effects
			UTIL_BloodDrips(vecPos, randomVec, BloodColor(), 5); // Couldn't really see?
			UTIL_BloodImpact(vecPos, randomVec, BloodColor(), 5); // Standard getting shot effect
			SpawnBlood(vecPos, randomVec, BloodColor(), info.GetDamage() * 3); //Idk what this does lets see
...
{
	CTakeDamageInfo subinfo = inputInfo;
	Vector vecDamageDir = inputInfo.GetDamageForce();
	
	EmitSound("Player.Gib");

	//Blood
	for (int i = 0; i < random->RandomInt(1, 2); i++) // Random meatiness?
	{
		...
		UTIL_BloodSpray(vecPos, randomVecTwo, BloodColor(), 1, FX_BLOODSPRAY_ALL); //Second set of effects
		UTIL_BloodDrips(vecPos, randomVec, BloodColor(), 1); // Couldn't really see?
		UTIL_BloodImpact(vecPos, randomVec, BloodColor(), 1); // Standard getting shot effect
		SpawnBlood(vecPos, randomVec, BloodColor(), inputInfo.GetDamage() ); //Idk what this does lets see
