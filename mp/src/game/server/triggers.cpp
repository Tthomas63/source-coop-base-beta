...
void CChangeLevel::ChangeLevelNow( CBaseEntity *pActivator )
{
	...
	if (!pPlayer) // Not valid player? Dont change
		return;
...
////	Msg( "Level touches %d levels\n", ChangeList( levels, 16 ) );
	...
	{
		Msg("Attempting level change.\n");

		// Cat two strings
		char strChangeOne[200]; // Idfk how long this string will be
		char strChangeTwo[200]; // Idfk how long this string will be
		char str1[] = "changelevel ";
		//strcat(str1, st_szNextMap);

		// strChangeOne should now hold 'changelevel <mapname>'
		strcpy(strChangeOne, strcat(str1, st_szNextMap));
		strcat(strChangeOne, " ");


		
		//strcat(str, st_szNextSpot);
		// strChangeTwo should now hold strChangeOne + NextSpot = 'changelevel <mapname> <spot>'
		strcpy(strChangeTwo, strcat(strChangeOne, st_szNextSpot));
		strcat(strChangeTwo, "\n");

		// Below calls changelevel 2? We want changelevel 1.
		//engine->ChangeLevel( st_szNextMap, st_szNextSpot );
		//std::string pTempConCommand = "changelevel" + std::to_string(st_szNextMap) + std::to_string(st_szNextSpot)
		engine->ServerCommand(strChangeTwo);
	}
	else
	{
