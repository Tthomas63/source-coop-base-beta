...
void CGib::SpawnStickyGibs( CBaseEntity *pVictim, Vector vecOrigin, int cGibs )
{
	...
	for ( i = 0 ; i < cGibs ; i++ )
	{
		...
		pGib->Spawn("models/gibs/pgib_p1.mdl" );
		pGib->Spawn("models/gibs/rgib_p2.mdl");
								...
								float			flLifetime)
{
	for (int i=0;i<nNumGibs;i++)
	{
		...
		//Don't know if this hacky shit will work
		pGib->SetThink(&CGib::SUB_FadeOut);
		pGib->SetNextThink(gpGlobals->curtime + pGib->m_lifeTime);

		pGib->RegisterThinkContext("BleedContext");
		pGib->SetContextThink(&CGib::BleedThink, gpGlobals->curtime, "BleedContext");
...
// Custom to take an extra arg specifying blood color
//------------------------------------------------------------------------------
// Purpose : Given an .mdl file with gibs and the number of gibs in the file
//			 spawns them in pVictim's bounding box
// Input   :
// Output  :
//------------------------------------------------------------------------------
void CGib::CustomSpawnSpecificGibs(CBaseEntity*	pVictim,
	int				nNumGibs,
	float			vMinVelocity,
	float			vMaxVelocity,
	const char*		cModelName,
	float			flLifetime,
	int				nCustomBloodColor)
{
	for (int i = 0; i<nNumGibs; i++)
	{
		CGib *pGib = CREATE_ENTITY(CGib, "gib");
		...
		// As the very last thing we set our blood color :)

		pGib->SetBloodColor(nCustomBloodColor);
...
void CGib::WaitTillLand ( void )
{
	...
	if ( GetAbsVelocity() == vec3_origin )
	{
		...
		// If you bleed, you stink! // Not getting called
...
void CGib::BleedThink ( void )
{
	...
	SpawnBlood(GetAbsOrigin(), vecDir, m_bloodColor, 100); //Idk what this does lets see

	// Put blood on the ground if near enough
	// Below code is from the barnacle example. In this, it is assumed the barnacle is upside down (?), so it has a range of 256?
	trace_t bloodTrace;
	//UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 256), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &bloodTrace);
	UTIL_TraceLine(GetAbsOrigin() + Vector(0,0,50), GetAbsOrigin() - Vector(0, 0, 256), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &bloodTrace);


	if (bloodTrace.fraction < 1.0f)
	{
#ifdef _XBOX
		UTIL_BloodDecalTrace(&bloodTrace, m_bloodColor);
#else
		UTIL_BloodDecalTrace(&bloodTrace, m_bloodColor);
#endif
	}
	
	SetNextThink(gpGlobals->curtime + random->RandomFloat(3.0f, 6.0f), "BleedContext");
}
...
void CGib::BounceGibTouch ( CBaseEntity *pOther )
{
	...
	else
	{
		if ( g_Language.GetInt() != LANGUAGE_GERMAN && m_cBloodDecals > 0 && m_bloodColor != DONT_BLEED )
		{
			...
			UTIL_BloodSpray(vecSpot, vecSpot + Vector(0, 0, 24), BLOOD_COLOR_RED, 50, FX_BLOODSPRAY_ALL);
			for (int i = 0; i < 32; i++)
			{
				vecSpot.x = vecSpot.x + random->RandomFloat(-16.0f, 16.0f);
				vecSpot.y = vecSpot.y + random->RandomFloat(-16.0f, 16.0f);
				vecSpot.z = vecSpot.z + 14;

				UTIL_TraceLine(vecSpot, vecSpot + ((vecSpot + Vector(0, 0, 24))), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
				UTIL_BloodSpray(vecSpot, vecSpot + Vector(0, 0, 24), BLOOD_COLOR_RED, 170, FX_BLOODSPRAY_ALL);
				UTIL_BloodSpray(vecSpot, vecSpot + Vector(0, 0, -24), BLOOD_COLOR_RED, 170, FX_BLOODSPRAY_ALL);
				UTIL_BloodSpray(vecSpot, vecSpot + Vector(0, 0, -24), BLOOD_COLOR_RED, 170, FX_BLOODSPRAY_ALL);
				UTIL_BloodSpray(vecSpot, vecSpot + Vector(0, 0, -24), BLOOD_COLOR_RED, 170, FX_BLOODSPRAY_ALL);
				UTIL_BloodSpray(vecSpot, vecSpot + Vector(0, 0, -24), BLOOD_COLOR_RED, 170, FX_BLOODSPRAY_ALL);
				if (tr.fraction != 1.0)
				{
					UTIL_BloodDecalTrace(&tr, BLOOD_COLOR_RED); // Change to RED?
					UTIL_BloodDecalTrace(&tr, BLOOD_COLOR_RED); // Change to RED?
					UTIL_BloodDecalTrace(&tr, BLOOD_COLOR_RED); // Change to RED?
					UTIL_BloodDecalTrace(&tr, BLOOD_COLOR_RED); // Change to RED?
					UTIL_BloodDecalTrace(&tr, BLOOD_COLOR_RED); // Change to RED?
			...
			for (int i = 0; i < 32; i++)
			{
				...
				UTIL_BloodDrips(vecSpot, vecSpot, BLOOD_COLOR_RED, 255);
			}
...
void CGib::Spawn( const char *szGibModel )
{
	...
	m_material = matNone;
	m_cBloodDecals = 500;// how many blood decals this gib can place (1 per bounce until none remain). 
